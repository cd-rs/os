...
- The default configuration of the PICs sends interrupt vector numbers in the range of 0–15 to the CPU. 
- These numbers are already occupied by (the first `0x20`) CPU exceptions. 
- For example, number 8 corresponds to a *double fault* in the CPU and *Serial Port 1* in the PIC.
- We need to **re**map the PIC interrupts to different numbers. 
